...
def cartDisplay(windowWidth : int,windowHeight : int,user:dict):
    ...
    if(len(cartData)>0):
        for data in cartData:
            ...
            layout1 += [
                [sg.Text('Profile\t\t',**style.normal),sg.Column([[sg.Text(text=data['title'].title(),key=data['title'],size=(20,1),**style.normal)],[sg.Text(text=data['harga'],key=f"HARGA {key}",size=(20,1),**style.normal)]]),sg.Text(text='\t' +str(data['count']) + '\t',key=f'COUNT {key}',size=(10,1),**style.normal),sg.Text(text=price + '\t', key=f'HARGATOTAL {key}',size=(20,1),**style.normal),sg.Button(button_text='-',key=f"REDUCE {key}",**style.bd),sg.Button(button_text='+',key=f"ADD {key}",**style.bd)],
                [sg.Text('\n\n')]
                ]
    else:
